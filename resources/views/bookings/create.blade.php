...
@php
    $paymentInfo = [
        'immediate' => 'Pay online (M-Pesa or card) after booking.',
        'venue' => 'Reserve now, pay at the gate on event day.',
        'both' => 'Pay online now (M-Pesa/card) or pay at the gate on event day.',
    ];
    $pm = $event->payment_mode ?? 'both';
    $paymentNote = $paymentInfo[$pm] ?? $paymentInfo['both'];
@endphp
<div class="max-w-lg w-full">
    <div class="bg-white border border-slate-200 overflow-hidden rounded-2xl sm:rounded-none">
        <div class="p-6 border-b border-slate-200 bg-slate-50 border-l-4 border-l-accent">
            ...
            <p class="mt-1 text-base text-slate-500">{{ $event->venue }} · {{ $event->event_date->format('M j, Y') }}</p>
            <p class="mt-3 text-sm text-slate-600 font-medium">{{ $paymentNote }}</p>
        </div>
        <form action="{{ route('bookings.store') }}" method="POST" class="p-6" id="bookingForm">
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <select name="ticket_type_id" required class="w-full border border-slate-300 px-3 py-2.5 text-base focus:ring-2 focus:ring-accent focus:border-accent" id="ticketType">
                ...
                <div>
                    ...
                    <input type="number" name="quantity" min="1" required class="w-full border border-slate-300 px-3 py-2.5 text-base focus:ring-2 focus:ring-accent focus:border-accent" placeholder="1" id="quantity" value="{{ old('quantity', 1) }}">
                    <p class="mt-1 text-sm text-slate-500">Max per type shown in dropdown</p>
                </div>
            ...
            <button type="submit" class="mt-6 w-full py-4 bg-navy-900 text-white text-base font-semibold hover:bg-navy-800 active:bg-navy-700 transition min-h-[52px] rounded-xl sm:rounded-none">
                {{ $pm === 'venue' ? 'Reserve tickets (pay at gate)' : 'Continue to payment' }}
            </button>
        </form>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateTotal() {
        ...
        if (qty > max) { qtyInput.value = max; qty = max; }
