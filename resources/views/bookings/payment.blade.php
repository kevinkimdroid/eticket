...
<div class="max-w-md w-full mx-auto">
    <div class="bg-white border border-slate-200 overflow-hidden rounded-2xl sm:rounded-none">
        ...
        <form action="{{ route('bookings.pay', $booking) }}" method="POST" class="p-6">
            @csrf
            @php $mode = $booking->event->payment_mode ?? 'both'; @endphp
            <div class="space-y-2">
                @if(in_array($mode, ['both', 'immediate']) && config('services.mpesa.consumer_key'))
                <label class="flex items-center gap-4 p-4 border border-slate-200 cursor-pointer hover:border-slate-300 transition has-[:checked]:border-accent has-[:checked]:bg-accent/5">
                    <input type="radio" name="payment" value="mpesa" class="w-4 h-4 text-accent" checked>
                    <div class="flex-1">
                        <span class="font-medium text-slate-900">M-Pesa</span>
                        <p class="text-sm text-slate-500">Pay with your phone</p>
                    </div>
                </label>
                @endif
                ...
                <label class="flex items-center gap-4 p-4 border border-slate-200 cursor-pointer hover:border-slate-300 transition has-[:checked]:border-accent has-[:checked]:bg-accent/5">
                    ...
                    <div class="flex-1">
                        <span class="font-medium text-slate-900">Card (Visa/Mastercard)</span>
                        <p class="text-sm text-slate-500">Pay with Stripe</p>
                    </div>
                </label>
                @endif
                @if($mode === 'both')
                <label class="flex items-center gap-4 p-4 border border-slate-200 cursor-pointer hover:border-slate-300 transition has-[:checked]:border-accent has-[:checked]:bg-accent/5">
                    <input type="radio" name="payment" value="cash" class="w-4 h-4 text-accent" {{ !config('services.stripe.key') && !config('services.mpesa.consumer_key') ? 'checked' : '' }}>
                    <div class="flex-1">
                        <span class="font-medium text-slate-900">Pay at venue</span>
                        <p class="text-sm text-slate-500">Cash on arrival</p>
                    </div>
                </label>
                @endif
            </div>
            ...
            @endif
            <button type="submit" class="w-full mt-6 py-4 bg-navy-900 text-white text-sm font-semibold hover:bg-navy-800 active:bg-navy-700 transition min-h-[52px] rounded-xl sm:rounded-none">
                Complete purchase
            </button>
        </form>
    </div>
